...
namespace Azul
{
	...
	//Trace::out("Cam location: %f %f %f\n", this->vPos[x], this->vPos[y], this->vPos[z]);
	...
	void Camera::getFieldOfView(float &angleRadians) const
	{
		// in radians
		angleRadians = this->fovy;
	};

	void Camera::setFieldOfView(const float angleRadians)
	{
		// in radians
		this->fovy = angleRadians;
	};
