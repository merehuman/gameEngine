...
#pragma pack(8)
	...
	void CreateTextureGLB(const char* const pFileName, const char* const pTargetName)
	{
		bool status;
		tinygltf::Model gltfModel;

		// Runtime texture model
		textureData runTexture;
		...
		int textureIndex = gltfModel.materials[0].pbrMetallicRoughness.baseColorTexture.index;
		...
		runTexture.dataSize = bufferView.byteLength;
		runTexture.poData = new unsigned char[runTexture.dataSize];
		memcpy(runTexture.poData, &buffer.data[bufferView.byteOffset], runTexture.dataSize);

		//write texture file
		std::string outputFileName = std::string(pTargetName) + ".png";
		Trace::out(" --> %+30s\n", outputFileName.c_str());

		File::Handle fh = nullptr;
		File::Error err = File::Open(fh, outputFileName.c_str(), File::Mode::WRITE);
		assert(err == File::Error::SUCCESS);

		File::Write(fh, runTexture.poData, runTexture.dataSize);
		assert(err == File::Error::SUCCESS);
	...
	//	textData.poData = (unsigned char *)pBits;
		...
		int req_comp = 4;  // request 4 components (RGBA)
		
		unsigned char* pBits = stbi_load(pFileName, &Width, &Height, &comp, req_comp);
		
		if (!pBits) {
			assert(false && "Failed to load TGA texture with stbi_load");
		}
		
		// set the format based on the number of channels
		textureData::TEXTURE_EFORMAT eFormat = (comp == 3)
		? textureData::TEXTURE_EFORMAT::EFORMAT_RGB
			: textureData::TEXTURE_EFORMAT::EFORMAT_RGBA;
		textureData::TEXTURE_NCOMPONENT nComponents = (comp == 3)
		? textureData::TEXTURE_NCOMPONENT::NCOMPONENT_RGB
			: textureData::TEXTURE_NCOMPONENT::NCOMPONENT_RGBA;
		...
		textData.dataSize = (unsigned long)(Width * Height * req_comp);
		textData.poData = pBits;
	}
