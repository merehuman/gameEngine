...
struct VariantKey {
  ...
  size_t Hash() const {
    // Keenan return data == nullptr ? std::hash<uint64_t>{}(integral)
  ...
  friend bool operator<(const VariantKey& left, const VariantKey& right) {
    ...
    // Keenan: return memcmp(left.data, right.data, left.integral) < 0;
...
class KeyMapBase : public UntypedMapBase {
 ...
 protected:
  ...
  void Resize(map_index_t new_num_buckets) {
    ...
    // Keenan ABSL_DCHECK_GE(new_num_buckets, kMinTableSize);
    ABSL_DCHECK_GE(new_num_buckets, (uint32_t) kMinTableSize);
